...
# header=T (TRUE) tells R that the first row of the data file contains the column names.
...
# Q1: How many rows and columns are there in the data set?
dim(errors)

# Q2: Number of students in class
sum(errors$size)/10  ## Is the total number of students 
sum(errors$females)/10 ## Is the total number of femals 
sum(errors$size)/10 - sum(errors$females)/10 # number of male students 

# Q3: Note down the number of photographed person for each unique value of age.
table(errors$tru_age)/55
## divided by 55 since each of the 55 groups guessed the photos 

# Q4: Cross tabulation 
table(errors$sex, errors$race)/55
## divided by 55 since each of the 55 groups guessed the photos 

# Q5: What are the minimum and maximum true ages of the photographed mathematicians?
# alternatively can find the minimums and maximums this way
min(errors$tru_age)
max(errors$tru_age)

# Q6. Obtain a barplot of the true age distribution.
barplot(table(errors$tru_age)/55)

# Q7: Obtain a histogram of the estimated age column and compare this with the true age distribution seen in the barplot drawn above.
hist(errors$est_age)

# Q8: Plot estimated age against true ages 

plot(errors$tru_age, errors$est_age)

# Q9. What are the means and standard deviations for the columns: size, females, est age, tru age, error and abs error?
summary(errors)
...
# Q10: What is the mean number of males in each group? What is the mean number of females in each group?
# mean number of males in each group - first, create a new variable containing the number of males in a group
errors$males <- errors$size - errors$female
head(errors$males) # to see the first few rows of the data set containing the new variable
mean(errors$males)

# mean number of females in each group
mean(errors$females)

# Q11. How many of the photographs were of each race?
table(errors$race)/55

# Q12. Note down the frequency table of the sign of the errors.
errors$sign <- sign(errors$error)
table(errors$sign)

# Q13. Obtain a histogram for the errors and another for the absolute errors. Which one is bellshaped and why?
hist(errors$error)
hist(errors$abs_error)


# Q14. Obtain a histogram for the square-root of absolute errors. 
hist(sqrt(errors$abs_error))


# Q15. Draw a boxplot
boxplot(errors$abs_error)


# Q16. Is it easier to guess the ages of female mathematicians?
errors$sex <- factor(errors$sex) # make sex a factor variable
levels(errors$sex)
tapply(X=errors$abs_error, INDEX=errors$sex, FUN =mean)

#Also draw a side by side boxplot of the absolute errors for the two groups of mathematicians: males and females.
boxplot(data=errors, abs_error~sex, col=c(2,4) )

# Q17. Is it easier to guess the ages of black mathematicians? 
tapply(X=errors$abs_error, INDEX=errors$race, FUN =mean)

# Q18 How would you order the mean absolute error by race?
boxplot(data=errors, abs_error~race, col=c(7, 8, 5, 0) )

# Q19. Is it easier to guess the ages of younger mathematicians?
tapply(X=errors$abs_error, INDEX=errors$tru_age, FUN =mean)

# Q20. Which person's age is the most difficult to guess?
boxplot(data=errors, abs_error~photo, col=heat.colors(8))
